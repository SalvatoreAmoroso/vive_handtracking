...
public class Teleport_Handtracking : MonoBehaviour
{
    ...
    //-------------------------------------------------
    private static Teleport_Handtracking _instance;
    public static Teleport_Handtracking instance
    {
        get
        {
            if (_instance == null)
            {
                _instance = FindObjectOfType<Teleport_Handtracking>();
            }
    ...
    private void Awake()
    {
        ...
        teleportArc = GetComponent<TeleportArc_Handtracking>();
    ...
    private void Start()
    {
        teleportMarkers = FindObjectsOfType<TeleportMarkerBase>(true);
    ...
    private void Update()
    {
        ...
        {
            bool prepared = WasTeleportPrepared(pointerHand);
            if (!visible && prepared)
            {
                //Begin showing the pointer
                ShowPointer(pointerHand);
            }
            else if (visible && state == 0)
            {
                //Hide the pointer
                HidePointer();
            }
    ...
    {
        if (GestureProvider.RightHand != null) //Todo: Nicht auf null prüfen, sondern auf die Geste -> so kann auch die zweite hand sichtbar sein
        {
    ...
    private void UpdatePointer()
    {
        ...
        if (teleportArc.DrawArc(out RaycastHit hitInfo))
        {
            hitSomething = true;
        ...
        if (debugFloor)
        {
            ...
            if (teleportArea != null)
            {
                if (floorFixupMaximumTraceDistance > 0.0f)
                {
                    ...
                    traceDir.x = 0.01f;
                    if (Physics.Raycast(pointedAtPosition + 0.05f * traceDir, traceDir, out RaycastHit raycastHit, floorFixupMaximumTraceDistance, floorFixupTraceLayerMask))
                    {
    ...
    //-------------------------------------------------
    private void ShowPointer(GameObject hand)
    {
        ...
        if (pointerHand)
        {
            pointerStartTransform = GetPointerStartTransform(isLeft);
    ...
    //-------------------------------------------------
    public bool IsEligibleForTeleport(GameObject hand)
    {
        if (hand == null || !hand.activeInHierarchy)
            return false;
        return true;
    }
    ...
    //-------------------------------------------------
    private bool WasTeleportTriggered(GameObject hand)
    {
        if (!IsEligibleForTeleport(hand) || state != 2) return false;
        return true;
    }
    ...
    //-------------------------------------------------
    private bool WasTeleportPrepared(GameObject hand)
    {
        if (!IsEligibleForTeleport(hand) || state != 1) return false;
        return true;
    }
